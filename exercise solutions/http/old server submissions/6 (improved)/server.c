 ...
 */
/**
 * @brief Generates a Response struct from the arguments and socket file
 * descriptor.
 * @param args Arguments struct containing the arguments.
 * @param reqfd file descriptor from socket.
...
/**
 * Don't refactor by using `fprintf()` instead of `write()`.
 *
 * Many test-cases use `wget` which only works with the functions `write()` and
