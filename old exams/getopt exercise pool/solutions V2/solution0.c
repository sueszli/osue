    ...
    while ( (c1 = getopt(argc,argv,"a:b:c::de")) != -1){
        switch(c1){
            case 'a':
                ...
                a_occured = true;
                a = optarg;  // how to throw exception, if optargA not occured? -> getopt should do. Test it!
                break;
            case 'b':
                if(b_occured) usage("passed in -b more than once.");
                b_occured = true;
                b = optarg; // how to throw exception, if optargB not occured? -> getopt should do. Test it!
                break;
            case 'c':
