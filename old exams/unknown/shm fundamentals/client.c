...
int main(int argc, char **argv) {
  ...
  // act, as if both got executed at the same time (cause the ... part is not
  // even necessary!) precondition: server got executed before client. (TODO:
  // can this be avoided)
  strcpy(shmp, "Hello World!");
  sem_post(sem_req);
  if (sem_wait(sem_res) == -1) error_exit("wait");
  printf(
      "server responded to request, which means shared memory can now be "
      "unmapped & semaphores be closed\n");
