...
void parse_arguments(int argc, char *argv[], args_t *args) {
  ...
  if (countp != 1) {
    usage("invalid number of optional arguments");
    exit(EXIT_FAILURE);
  }
...
void allocate_resources(void) {
  ...
  sem_request = sem_open(SEM_NAME_REQUEST, O_RDWR | O_CREAT);
  sem_response = sem_open(SEM_NAME_RESPONSE, O_RDWR | O_CREAT);
  sem_client = sem_open(SEM_NAME_CLIENT, O_RDWR | O_CREAT);
